...
// Dev mode toggle
export const [devMode, setDevMode] = createSignal<boolean>(() => {
  const stored = localStorage.getItem('wheelgen-dev-mode');
  return stored === 'true';
});

// Toggle dev mode and persist to localStorage
export const toggleDevMode = () => {
  const newMode = !devMode();
  setDevMode(newMode);
  localStorage.setItem('wheelgen-dev-mode', newMode.toString());
};
...
export const updateRingPattern = (index: number, pattern: string, p: p5) => {
  ...
  if (pattern === '') {
    ...
    // Check if this is a pattern language expression
    if (isPatternLanguage(pattern)) {
      const result = parsePattern(pattern);
      if (result.success && result.expanded) {
        // Convert expanded pattern back to legacy format for Ring.setPattern
        const legacyPattern = result.expanded.map(item => {
          let str = item.rotated ? item.char.toUpperCase() : item.char;
          return str;
        }).join('');
        
        ring.setPattern(p, legacyPattern);
        console.log(`Pattern language: "${pattern}" → "${legacyPattern}"`);
      } else {
        console.error('Pattern language parse error:', result.error);
