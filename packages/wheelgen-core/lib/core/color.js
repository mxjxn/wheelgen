export function generatePalette(p) {
 ...
 */
export function generateDirectRgbPalette(p) {
    const palette = [];
 ...
 */
function generateLegacyPalette(p) {
    const palette = [];
    const prevMode = p._colorMode; // best-effort; p5 lacks public getter
    p.colorMode(p.HSB, 360, 100, 100);
    const baseHue = p.random(360);
    for (let i = 0; i < 4; i++) {
        const hue = (baseHue + i * 90) % 360;
        palette.push(p.color(hue, p.random(60, 90), p.random(80, 100)));
    }
...
export function createHSBColor(p, h, s, b) {
...
export function colorToRgbString(p, color) {
    // Store current color mode
    const currentMode = p._colorMode;
    const currentMaxes = p._colorMaxes;
    try {
...
export function batchConvertColorsToRgb(p, colors) {
    if (colors.length === 0)
        return [];
    // Store current color mode
    const currentMode = p._colorMode;
...
export function hsbToRgb(p, h, s, b) {
    if (s <= 5) {
...
export function logColorInfo(p, color, label) {
    // Store current color mode
    const currentMode = p._colorMode;
    const currentMaxes = p._colorMaxes;
    try {
...
export function logPaletteColors(p, palette) {
