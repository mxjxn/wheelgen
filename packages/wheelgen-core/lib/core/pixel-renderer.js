export class PixelStrokeRenderer {
    constructor() {
        this.strokeTextures = new Map();
        this.maxCacheSize = 100; // Maximum number of cached textures
        this.cacheHits = 0;
        this.cacheMisses = 0;
    }
    /**
     ...
     */
    getTextureKey(options) {
        const { w, h, strokeWidth, strokeCount, colorBleed, strokeType, curveIntensity = 0, upwardLength = 1.0, length = 1.0, size = 1.0 } = options;
     ...
     */
    generateStrokeTexture(p, options) {
     ...
     */
    generateDiamondTexture(buffer, options, width, height) {
     ...
     */
    generateHorizontalHookTexture(buffer, options, width, height) {
        ...
        for (let stroke = 0; stroke < strokeCount; stroke++) {
            ...
            for (let i = 0; i < segments; i++) {
                ...
                // Draw line segment
                this.drawPixelLine(buffer, x1 + centerX, y1 + centerY, x2 + centerX, y2 + centerY, strokeR, strokeG, strokeB, strokeWidth, width, height);
            }
        }
        buffer.updatePixels();
        buffer.colorMode(buffer.HSB, 360, 100, 100);
     ...
     */
    generateLStrokeTexture(buffer, options, width, height) {
        ...
        for (let stroke = 0; stroke < strokeCount; stroke++) {
            ...
            this.drawPixelLine(buffer, centerX - effectiveWidth / 2, baseY, centerX + effectiveWidth / 2, baseY, strokeR, strokeG, strokeB, strokeWidth, width, height);
            // Draw vertical stem
            ...
            this.drawPixelLine(buffer, centerX - effectiveWidth / 2, baseY, stemEndX, stemEndY, strokeR, strokeG, strokeB, strokeWidth, width, height);
        }
        buffer.updatePixels();
        buffer.colorMode(buffer.HSB, 360, 100, 100);
    }
     ...
     */
    generateVerticalHookTexture(buffer, options, width, height) {
     ...
     */
    generateSolidRingTexture(buffer, options, width, height) {
        ...
        for (let stroke = 0; stroke < strokeCount; stroke++) {
            ...
            this.drawPixelCircle(buffer, centerX, centerY, strokeRadius, strokeR, strokeG, strokeB, strokeWidth, width, height);
        }
        buffer.updatePixels();
        buffer.colorMode(buffer.HSB, 360, 100, 100);
    }
     ...
     */
    drawPixelLine(buffer, x1, y1, x2, y2, r, g, b, strokeWidth, width, height) {
     ...
     */
    drawPixelCircle(buffer, centerX, centerY, radius, r, g, b, strokeWidth, width, height) {
     ...
     */
    renderStroke(p, options, x, y, progress = 1.0) {
...
export function getPixelRenderer() {
    if (!pixelRenderer) {
