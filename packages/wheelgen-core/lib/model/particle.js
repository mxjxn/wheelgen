...
export class Particle {
    ...
    updateStrokeData(p) {
        ...
        for (let i = 0; i < currentPalette.length; i++) {
            ...
            const hueDiff = Math.min(Math.abs(paletteHue - currentHue), 360 - Math.abs(paletteHue - currentHue) // Handle wraparound
            );
        ...
        for (let i = 0; i < numLinesInNib; i++) {
            ...
            const strokeColor = p.lerpColor(strokeBaseColor, adjacentColor, bleedT);
