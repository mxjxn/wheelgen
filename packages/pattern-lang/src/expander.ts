...
export function patternToGrammarString(pattern: any): string {
  if (typeof pattern === 'string') return pattern;
  
  if (pattern.type === 'command') {
    const args = pattern.args.map((arg: any) => patternToGrammarString(arg)).join(', ');
    return `${pattern.name}(${args})`;
  }
  
  if (pattern.type === 'sequence') {
    const patterns = pattern.patterns.map((p: any) => patternToGrammarString(p)).join('');
    return `$${patterns}`;
  }
