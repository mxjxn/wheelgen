...
export class ColorDiagnostic {
   ...
   */
  private testPaletteColor(color: p5.Color, index: number): void {
   ...
   */
  private testUIColorConversion(color: p5.Color): string {
    // Store current color mode
    const currentMode = (this.p as any)._colorMode;
    const currentMaxes = (this.p as any)._colorMaxes;
   ...
   */
  private testRoundTripConversion(originalColor: p5.Color): void {
    this.p.colorMode(this.p.HSB, 360, 100, 100);
   ...
   */
  private testColorLerping(palette: p5.Color[]): void {
    ...
    bleedValues.forEach(bleedT => {
      this.p.colorMode(this.p.HSB, 360, 100, 100);
      const lerpedColor = this.p.lerpColor(strokeBaseColor, adjacentColor, bleedT) as p5.Color;
   ...
   */
  private testConversionConsistency(palette: p5.Color[]): void {
   ...
   */
  private getColorString(color: p5.Color): string {
    this.p.colorMode(this.p.RGB, 255);
    const r = Math.round(this.p.red(color));
    const g = Math.round(this.p.green(color));
    const b = Math.round(this.p.blue(color));
 ...
 */
export function testSaveLoadColorConsistency(p: p5): void {
 ...
 */
export function testActualSaveLoadSystem(p: p5): void {
 ...
 */
export function testNewColorGeneration(p: p5): void {
 ...
 */
export function runQuickColorDiagnostic(p: p5): void {
